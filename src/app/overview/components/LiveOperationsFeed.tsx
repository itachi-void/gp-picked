...
const getPriority = (p: keyof typeof PRIORITY_ORDER) =>
  PRIORITY_ORDER[p];

export default function LiveOperationsFeed() {
  const router = useRouter();
  const { requests, isLoading, error, fetchRequests } = usePickup();
  ...
  useEffect(() => {
    if (error) console.error("Pickup error:", error);
  }, [error]);

  // فرز الطلبات حسب الأولوية
  ...
  const topRequests = sorted.slice(0, 5);
  ...
  return (
    <GlassCard className="p-6 h-full flex flex-col">
      ...
      {/* Content Area */}
      {isLoading ? (
        /* List loading skeleton to prevent Layout Shift */
        <div className="space-y-2 flex-1 animate-pulse">
          {[...Array(5)].map((_, i) => (
            <div
              key={i}
              className="w-full p-3 rounded-2xl bg-white/40 dark:bg-white/[0.02] border border-slate-200/50 dark:border-white/5 flex gap-3 h-[62px]"
            >
              <div className="w-2 h-2 rounded-full bg-slate-300 dark:bg-slate-700 mt-1 shrink-0" />
              <div className="flex-1 space-y-2 py-0.5">
                <div className="h-4 bg-slate-300 dark:bg-slate-700 rounded w-1/4" />
                <div className="h-3 bg-slate-200 dark:bg-slate-800 rounded w-1/2" />
              </div>
              <div className="h-5 bg-slate-300 dark:bg-slate-700 rounded w-12" />
            </div>
          ))}
        </div>
      ) : (
        ...
        <div className="space-y-2 flex-1">
          {topRequests.map((r, i) => {
            ...
            const weight = r.items.reduce((sum, it) => sum + it.expectedWeightKg, 0);
            return (
              <button
                key={r.id}
                onClick={() => router.push("/pickups")}
