...
export default function ReportsPage() {
  ...
  const handleDownload = async (report: Report) => {
    ...
    try {
      ...
      let columns: { key: string; label: string; accessor: (row: any) => any }[] = [];

      if (Array.isArray(data)) {
        formattedRows = data;
        if (data.length > 0) {
          columns = Object.keys(data[0]).map(k => ({ key: k, label: k.toUpperCase(), accessor: (row: any) => row[k] }));
        }
      } else {
        formattedRows = [{ value: data }];
        columns = [{ key: "value", label: "RESULT", accessor: (row: any) => row.value }];
      }
              ...
              { title: "Email Reports", description: "Send to stakeholders", icon: Mail, accent: "emerald", onClick: () => setShowEmailDialog(true) },
              {
                ...
                onClick: () => {
                  exportToCsv("reports", filteredReports, [
                    { key: "id", label: "ID", accessor: (r: Report) => r.id },
                    { key: "name", label: "Name", accessor: (r: Report) => r.name },
                    { key: "type", label: "Type", accessor: (r: Report) => r.type },
                    { key: "status", label: "Status", accessor: (r: Report) => r.status },
                    { key: "createdDate", label: "Created", accessor: (r: Report) => r.createdDate },
                    { key: "generatedBy", label: "Generated By", accessor: (r: Report) => r.generatedBy },
                    { key: "fileSize", label: "Size", accessor: (r: Report) => r.fileSize }
                  ]);
