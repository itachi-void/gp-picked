    ...
    <div className="p-6 text-black dark:text-white transition-colors duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {sortedZones.map((zone) => {
          ...
          return (
            <div key={zone.id} className="relative overflow-hidden p-5 pl-6 rounded-2xl bg-white/70 dark:bg-white/[0.03] backdrop-blur-xl border border-slate-200/70 dark:border-white/10 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all">
              <span className={`absolute left-0 top-0 bottom-0 w-1 ${s.bar}`} />
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-white text-lg leading-tight tracking-tight">{zone.name}</h3>
                  <p className="text-slate-400 dark:text-white/40 text-xs uppercase tracking-wider font-semibold mt-0.5">{zone.city}</p>
                </div>
                <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold ${s.bg}`}>
                  <s.Icon size={12} /> {achievementRate > 0 ? "+" : ""}{achievementRate}%
                </div>
              </div>

              <div className="flex justify-between items-baseline mb-1.5">
                <span className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">{zone.collected}</span>
                <span className="text-[11px] text-slate-400 dark:text-white/40 font-semibold">/ {zone.target} target</span>
              </div>

              <div className="h-1.5 w-full bg-slate-100 dark:bg-white/5 rounded-full overflow-hidden mb-4">
                <div className={`h-full ${s.bar} rounded-full transition-all`} style={{ width: `${Math.min(100, (zone.collected/zone.target)*100)}%` }} />
              </div>

              <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-wider">
                <span className="inline-flex items-center gap-1 text-slate-500 dark:text-white/60">
                  <span className="w-1.5 h-1.5 bg-cyan-500 rounded-full" />
                  {zone.drivers} drivers
                </span>
                <span className={`inline-flex items-center gap-1 ${zone.pending > 5 ? "text-red-600 dark:text-red-400" : "text-slate-500 dark:text-white/60"}`}>
                  <span className={`w-1.5 h-1.5 rounded-full ${zone.pending > 5 ? "bg-red-500" : "bg-slate-400"}`} />
